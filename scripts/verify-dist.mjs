...
const {base:configuredBase}=resolveSiteConfig(); const base=configuredBase==='/'?'/':`${configuredBase.replace(/\/$/,'')}/`;
...
const htmlFiles=[]; function walk(dir){for(const entry of fs.readdirSync(dir,{withFileTypes:true})){const full=path.join(dir,entry.name);if(entry.isDirectory())walk(full);else if(entry.name.endsWith('.html'))htmlFiles.push(full)}} walk(dist);
...
console.log(`Verified ${htmlFiles.length} HTML files, 3 locale trees, trailing-slash-safe locale navigation, localized lang metadata and required portal/docs routes.`);
