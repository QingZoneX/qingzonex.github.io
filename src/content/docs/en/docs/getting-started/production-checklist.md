...
`v0.1.1-alpha` is an Open Source Preview. Before exposing QTable to real users, complete these checks in an environment that matches your production topology.

## Version and source

- [ ] `qtable-server` and `qtable-web` use the `v0.1.1-alpha` tags or exact commits that were validated together.
...
- [ ] Public source visibility or a Git tag is not being mistaken for a published GitHub Release or stable artifact.
