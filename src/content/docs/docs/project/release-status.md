---
title: 发布状态
description: "QTable 当前公开源码状态、Alpha 标签基线与发行制品边界。"
---

QTable 当前处于 **`v0.1.1-alpha` — Open Source Preview**。

## 公开源码与标签

两个实现仓库目前都已公开，并且都存在 `v0.1.1-alpha` 标签：
...
当前两个仓库都没有已发布的 GitHub Release。因此，`v0.1.1-alpha` 标签表示当前公开源码 Alpha 基线，**不应被解读为已经发布了正式 GitHub Release、Docker Hub 镜像或稳定版发行物**。任何镜像或 Release 的存在都应以对应仓库的实际发布记录为准。
