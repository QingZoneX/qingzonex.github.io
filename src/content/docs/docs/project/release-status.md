---
title: 发布状态
description: "QTable 当前公开源码状态、Alpha Tag 与发行制品边界。"
---

QTable 当前公开版本为 **`v0.1.1-alpha` — Open Source Preview**。

## 公开源码与 Tag

两个实现仓库目前都已公开，并且都存在 `v0.1.1-alpha` Git tag：
...
截至当前，两个仓库都没有与 `v0.1.1-alpha` 对应的已发布 GitHub Release。因此，**Git tag 已发布不应被解读为已经发布了 GitHub Release、Docker Hub 镜像或稳定版发行物**。任何镜像、Release 或其他发行制品的存在都应以对应仓库的实际发布记录为准。
